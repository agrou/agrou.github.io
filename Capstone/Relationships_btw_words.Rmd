...
```{r}
# code to add to the previous code 
# below we separate the data into two columns in order to be able to use filters of stop words and strange words
...
BiGrams_filtered <- BiGrams_sep %>%
        filter(str_detect(word1, "[A-z]{1,}")) %>%
        filter(str_detect(word2, "[A-z]{1,}")) %>%
...
```{r}
# tests
word1 <- c("aaa", "a_ss", "s12ffg", "sss-sss", "s^ss`sà")
word1
str_view(word1, "[A-z]{1,}")
```
